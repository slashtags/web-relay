...
class Record {
  ...
  [Symbol.for('nodejs.util.inspect.custom')] () {
    return this.constructor.name + ' ' + JSON.stringify({
      signature: b4a.toString(this.signature, 'hex'),
      hash: b4a.toString(this.hash, 'hex'),
      timestamp: this.timestamp,
      metadata: this.metadata || null,
      base64: this.serialize('base64')
    }, null, 2)
  }
  ...
  verify (path) {
    ...
    // In the absence of key delegation, the publicKey is assumed to be the decoded userID
