...
class Relay {
  ...
  constructor (storage, options = {}) {
    ...
    // Add a unique prefix to avoid collisions when running multiple relays in unit tests
    const uniquePrefix = process.env.BRITTLE ? 'TEST_' + Math.random().toString(16).slice(2) + '_' : ''

    this._responseTimeHistogram = new Prometheus.Histogram({
      name: uniquePrefix + 'response_time',
      help: 'Response time in milliseconds',
      labelNames: ['method', 'route', 'status']
    })

    this._requestsCounter = new Prometheus.Counter({
      name: uniquePrefix + 'requests_count',
      help: 'Number of requests',
      labelNames: ['method', 'route', 'status']
    })

    Prometheus.collectDefaultMetrics({ prefix: uniquePrefix })
  }
   ...
   */
  stats () {
    const totalRecordsCount = this._recordsDB.getCount()
  ...
  _handle (req, res) {
    ...
    if (req.url.startsWith('/health-check') && req.method === 'GET') {
      this._HEALTH_CHECK(req, res)
    } else if (req.url.startsWith('/metrics') && req.method === 'GET') {
      this._METRICS(req, res)
    } else {
      this._handleRecordRequests(req, res)
    }

    this._responseTimeHistogram.observe({
      method: req.method,
      route: req.url,
      status: res.statusCode
    }, Date.now() - responseTimeStart)

    this._requestsCounter.inc({
      method: req.method,
      route: req.url,
      status: res.statusCode
    })
