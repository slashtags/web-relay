...
class WebRelayClient {
  ...
  async get (relayAddress, userID, path) {
    ...
    return new GetResponse(res, hasher, userID)
  }
...
class GetResponse {
   ...
   * @param {import('blake3').IHasher} hasher
   * @param {string} userID
   */
  constructor (response, hasher, userID) {
    this._response = response
    this._hasher = hasher

    this._hexContentHash = this._response.headers.get(HEADERS.CONTENT_HASH)
    this._base64Metadata = this._response.headers.get(HEADERS.METADATA)
    this._base64Signature = this._response.headers.get(HEADERS.SIGNATURE)

    const validSignature = verify({
      contentHash: b4a.from(this._hexContentHash, 'hex'),
      metadata: b4a.from(this._base64Metadata, 'base64'),
      signature: b4a.from(this._base64Signature, 'base64'),
      userID
    })
  ...
  async * [Symbol.asyncIterator] () {
    while (true) {
      /** @type {Uint8Array} */
      // @ts-ignore
      const chunk = this._response.body.read()

      this._hasher.update(chunk)

      if (chunk === null) {
        const hash = this._hasher.digest().toString('hex')

        this.valid = hash === this._hexContentHash

        try {
          /** @type {Metadata} */
          this.metadata = JSON.parse(b4a.toString(b4a.from(this._base64Metadata, 'base64')))
        } catch { }
