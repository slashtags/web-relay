...
class WebRelayClient {
  ...
  async get (relayAddress, userID, path) {
    ...
    const reader = response.body.on
      ? response.body //             Node
      // @ts-ignore
      : response.body.getReader() // Browser

    return new GetResponse(reader, hasher, metadata, hexContentHash)
  }
...
class GetResponse {
  /**
   * @param {import('node-fetch').Response['body']} reader
   * @param {import('blake3').IHasher} hasher
   * @param {Metadata} metadata
   * @param {string} hexContentHash
   */
  constructor (reader, hasher, metadata, hexContentHash) {
    this._reader = reader
    this._hasher = hasher
